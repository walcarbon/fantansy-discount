...
<div class="vcpl-table-menu">
	...
	/**
	 * Get allowed customer ids for current vendor context.
	 *
	 * @return array<int>
	 */
	private function get_allowed_customer_ids_for_current_vendor(): array {
		return array_values(
			array_filter(
				array_map( fn ( $customer ) => absint( $customer->ID ), vcpl_get_my_customers( $this->current_user_id ) ?: array() )
			)
		);
	}

	/**
	 * Resolve and validate posted customer id.
	 *
	 * @param  array<int> $allowed_customer_ids Allowed customer ids.
	 * @return int
	 */
	private function get_valid_posted_customer_id( array $allowed_customer_ids ): int {
		$raw_customer_order = isset( $_POST['customer_order'] ) ? wp_unslash( $_POST['customer_order'] ) : '';
		$customer_id        = is_numeric( $raw_customer_order ) ? absint( $raw_customer_order ) : 0;

		if ( $customer_id > 0 && in_array( $customer_id, $allowed_customer_ids, true ) ) {
			return $customer_id;
		}
	...
	public function customer_fields() {
		...
		$allowed_customer_ids = $this->get_allowed_customer_ids_for_current_vendor();
		...
		foreach ( $allowed_customer_ids as $customer_id ) {
			...
			if ( $customer ) {
				$options[ (string) $customer_id ] = $customer->user_login;
			}
		}
		...
		if ( isset( $_POST['customer_order'] ) ) {
			$selected_customer_id = $this->get_valid_posted_customer_id( $allowed_customer_ids );
		} else {
			$checkout_value       = absint( WC()->checkout->get_value( 'customer_order' ) );
			$selected_customer_id = in_array( $checkout_value, $allowed_customer_ids, true ) ? $checkout_value : 0;
		}
		...
		printf(
			...
			woocommerce_form_field(
				...
				$selected_customer_id > 0 ? (string) $selected_customer_id : ''
			)
		);
	}
	...
	public function validate_customer_assign_order() {
		...
		if ( 0 === $selected_customer_id ) {
			wc_add_notice( esc_html__( 'Please select a valid customer before placing the order.', VCPL_TEXT_DOMAIN ), 'error' );
		}
	}
